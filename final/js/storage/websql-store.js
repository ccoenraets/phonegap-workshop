var WebSqlStore = function(url, successCallback, errorCallback) {

    var self = this;
    $.ajax(url)
        .done(function(data) {
            self.initializeDatabase(data.employees, successCallback, errorCallback);
        });

    this.initializeDatabase = function(employees, successCallback, errorCallback) {
        var self = this;
        this.db = window.openDatabase("EmployeeDB", "1.0", "Employee Demo DB", 200000);
        this.db.transaction(
                function(tx) {
                    self.createTable(tx);
                    self.addEmployees(tx, employees);
                },
                function(error) {
    ...
    this.addEmployees = function(tx, employees) {
